...
class Controller {
  ...
  async stats(req, res, next) {
    try {
      ...
      if (email) {
        ...
        if (!Users.length) {
          return next(new HttpError(null, {}, NOT_FOUND, `Account not found for email ${email}.`));
        }
        const matchingVWs = await virtualWorldService.getAll({ cognitoUuid: Users[0].Username })
        if (!matchingVWs.length) {
          return next(new HttpError(null, {}, NOT_FOUND, `Virtual world not found for email ${email}.`));
        }
        where.vw = matchingVWs.map(vw => vw.id);
      }
      const capitalizedTimeFrame = timeFrame && timeFrame?.charAt(0).toUpperCase() + timeFrame?.slice(1);
      const totalProcesses = await dynamoService.getAll(where);
      totalProcesses.sort((a,b) =>  new Date(b.createdAt) - new Date(a.createdAt));
      
      if (!capitalizedTimeFrame) {
        return res.send(processesStats(totalProcesses));
      }
      
      let groupedResults = _.groupBy(totalProcesses, (result) => moment(result['createdAt'], 'DD/MM/YYYY').startOf(`${capitalizedTimeFrame}`));
  ...
  async errorsExport(req, res, next) {
    try {
      ...
      const totalProcesses = await dynamoService.getAll(where);
      totalProcesses.sort((a,b) =>  new Date(b.createdAt) - new Date(a.createdAt));
      const filtered = totalProcesses.filter((p) => ![0, 5, 6, 7].includes(p.mlReturnCode));
     
      const files = [{ data: JSON.stringify(filtered, null, '\t'), filename: 'manifest.json'}];
      
      await Promise.all(filtered.map(async(p) => {
        const ml = p?.mlReturnCode ? await schedulerService.getProcessInfos(p.uuid) : null;
        files.push({ data: JSON.stringify({...p, ml}, null, '\t'), filename: `${p.uuid}.json`});
        files.push({ key: `${p.video}-prepared.mp4`, filename: `${p.uuid}.mp4`});
      }));
      ...
      const stream = await awsService.archiveFilesStream(exportName, files, false, res);
      stream.pipe(res);
      stream.finalize();
      return res;
    } catch (e) {
  ...
  async listEmotes(req, res, next) {
    try {
      ...
      const response = await storeService.searchEmotes({ limit, offset, search });
  ...
  async listVirtualWorlds(req, res, next) {
    try {
      ...
      const { count, rows: virtualWorlds } = await vwService.getAndCountAll(where, ['*'], { limit, offset });
  ...
  async getActivity(req, res, next) {
    try {
      ...
      if (!virtualWorld) {
        return next(new HttpError(null, {}, NOT_FOUND, 'VirtualWorld not found.'));
      }
      ...
      res.send({
        emotes: {
          count: {
            ...
            backOffice: emoteSummary.filter(e => e.source === 'backOffice').length,
  ...
  async averageEmotesPerUser(req, res, next) {
    try {
      ...
      const count = await sequelize.query(averageEmotePerUser({ countOnly: true, virtualWorldUuids: vwUuids, userBlacklist}), { type: QueryTypes.SELECT });
      const result = await sequelize.query(averageEmotePerUser({limit, offset, orderBy, orderDirection, virtualWorldUuids: vwUuids, userBlacklist}), { type: QueryTypes.SELECT });
  ...
  async rankingUsersPerEmote(req, res, next) {
    try {
      const {limit, offset, vwUuids, emoteUuids, orderBy, orderDirection} = req.query;
      const userBlacklist = await awsService.getCognitoInternalBlacklist();
      const count = await sequelize.query(rankingUserPerEmote({virtualWorldUuids: vwUuids, emoteUuids, countOnly: true, userBlacklist}), {type: QueryTypes.SELECT});
      const result = await sequelize.query(rankingUserPerEmote({limit, offset, orderBy, orderDirection, virtualWorldUuids: vwUuids, emoteUuids, userBlacklist}), {type: QueryTypes.SELECT});
      const resultWithName = await Promise.all(result.map(async (r) => {
        try {
  ...
  async rankingEmotesPerGame(req, res, next) {
    try {
      const {limit, offset, vwUuids, orderBy, orderDirection, isUGE, isDeleted } = req.query;
      const userBlacklist = await awsService.getCognitoInternalBlacklist();
      const count = await sequelize.query(rankingEmotesPerVW({ virtualWorldUuids: vwUuids, isUGE, isDeleted, countOnly: true, userBlacklist }), {type: QueryTypes.SELECT});
      const result = await sequelize.query(rankingEmotesPerVW({limit, offset, orderBy, orderDirection,virtualWorldUuids: vwUuids, isUGE, isDeleted, userBlacklist}), {type: QueryTypes.SELECT});
      setPaginationHeaders(req, res, count[0].count, limit, offset);
      res.send({
        result,
      });
  ...
  async apiKeysPerVW(req, res, next) {
    try {
      const {limit = 5, offset = 0, vwUuids, orderBy, orderDirection, email} = req.query;
      const userBlacklist = await awsService.getCognitoInternalBlacklist();
      let whitelist = [];
      if (email) {
        const { Users } = await awsService.getCognitoUserByEmail(email);
      ...
      const count = await sequelize.query(apiKeysByVW({ ...queryParameters, countOnly: true}), {type: QueryTypes.SELECT});
      
      // First, fetch the cache to see all usages
      const caches = await cacheService.scan(`*usages:*`)
      const cachedUsages = await Promise.all(caches.map(async (rawKey) => {
        const key = rawKey.split(':').slice(1).join(':');
        return cacheService.get(key);
      }));
      const cachedValues = cachedUsages.reduce((acc, cur) => ({
        ...acc,
        [cur.lastUsage]: cur.keyId
      }), {})
      
      // Include formatted caches usages to query to fetch latest data
      const result = await sequelize.query(apiKeysByVW({ ...queryParameters, cachedValues}), {type: QueryTypes.SELECT});

      const resultWithEmails = await Promise.all(result.map(async(r) => ({
        ...r,
        email: r.cognito_uuid ? (await awsService.getCognitoUserInfo(r.cognito_uuid))?.email : '[Deleted User]',
  ...
  async userData(req, res, next) {
    try {
      const {limit = 5, offset = 0, orderBy, orderDirection, email} = req.query;
      const userBlacklist = await awsService.getCognitoInternalBlacklist();
      let whitelist = [];
      if (email) {
        const { Users } = await awsService.getCognitoUserByEmail(email);
      ...
      const count = await sequelize.query(rankingUserData({ ...queryParameters, countOnly: true}), {type: QueryTypes.SELECT});
      
      // Include formatted caches usages to query to fetch latest data
      const result = await sequelize.query(rankingUserData(queryParameters), {type: QueryTypes.SELECT});
      
      const resultWithEmails = await Promise.all(result.map(async(r) => {
        let userInfo
        try {
          userInfo = r.cognito_uuid ? await awsService.getCognitoUserInfo(r.cognito_uuid) : {};
        } catch (e) {
          logger.warn(e.message);
          userInfo = {}
        ...
        return {
          ...
          user_name: userInfo.given_name ? `${userInfo.given_name} ${userInfo.family_name}` : undefined
        }
      }));
      setPaginationHeaders(req, res, count[0].count, limit, offset);
      res.send({
  ...
  async aggregateTotal(req, res, next) {
    try {
      ...
      const accounts = await sequelize.query(cognitoAccountsWithUsageCount(userBlacklist), {type: QueryTypes.SELECT});
      const ugeEmotes = await sequelize.query(ugeEmotesCount(userBlacklist), {type: QueryTypes.SELECT});
      const ugeEmotesDeleted = await sequelize.query(ugeEmotesCount(userBlacklist, true), {type: QueryTypes.SELECT});
      const result = {
      ...
      const arrayResult = Object.entries(result).map(([label, value]) => ({ label, value }));
      res.send({
        result: arrayResult,
      });
    } catch (e) {
  ...
  async aggregateRatio(req, res, next) {
    try {
      const userBlacklist = await awsService.getCognitoInternalBlacklist();
      const gamesWithKey = await sequelize.query(gameWithKeyPercentage(userBlacklist), { type: QueryTypes.SELECT });
      const accWithUGR = await sequelize.query(accountsWithUGE(userBlacklist), { type: QueryTypes.SELECT });
      const accWithModeration = await sequelize.query(accountsWithModeration(userBlacklist), { type: QueryTypes.SELECT });
  ...
  async avatarsPerAccount(req, res, next) {
    try {
      ...
      const response = await storeService.getStatsAvatarsPerAccount({ limit, offset, orderBy, orderDirection });
      
      const resultWithEmails = await Promise.all(response.data.result.map(async(r) => {
        let email;
        try {
          email = r.cognito_uuid ? (await awsService.getCognitoUserInfo(r.cognito_uuid))?.email : '[Deleted User]'
        } catch (e) {
  ...
  async accountsWithAvatars(req, res, next) {
    try {
      const {limit, offset, orderBy, orderDirection} = req.query;
      const response = await storeService.getStatsAccountsWithAvatars({ limit, offset, orderBy, orderDirection });
  ...
  async impersonate(req, res, next) {
    try {
      ...
      await Promise.all(keys.map(key => cacheService.del(key.split(':').slice(1).join(':'))));
      await cacheService.set(`impersonate:${user.username}:${userToImpersonate.Username}`, true, 3600);
      res.send({
  ...
  async stopImpersonate(req, res, next) {
    try {
      ...
      const keys = await cacheService.scan(`impersonate:${adminUser.username}:*`);
      await Promise.all(keys.map(key => cacheService.del(key.split(':').slice(1).join(':'))));
      res.send({
        message: 'Done'
      });
  ...
  async exportStatsByEmail(req, res, next) {
    const getAvatarsByCognito = async (cognitoUuid) => {
      ...
      const avatarsTransformed = avatars.map(({ metadata, files, ...avatar }) => {
        ...
        avatar.retargetingQuality = metadata && metadata?.retargetingQuality === 'Premium' ? 'RT3K' : 'IK';
        return avatar;
      });
    ...
    const getVwsByCognito = async (cognitoUuid) => {
      ...
      const virtualWorlds = vws.map(({ dataValues: rawVW}) => {
        ...
        vw.plan = plan.name;
        Object.entries(configuration).forEach(([key, value]) => vw[key] = value);
        return vw;
      });
    ...
    const getTokensByCognito = async (cognitoUuid) => {
      const vws = await vwService.getAll({ cognitoUuid }, ['*'], { include: 
        [
          {
            model: Token,
            as: 'tokens',
    ...
    try {
      ...
      if (virtualWorlds.length > 0) {
        totalProcesses = await dynamoService.getAll({ vw: virtualWorlds.map((vw) => vw.id) });

        virtualWorlds.forEach((vw) => {
          vw.numberProcesses = totalProcesses.filter((p) => p.vw === vw.id).length;
        });
        ...
        players.forEach((g) => {
          g.numberProcesses = totalProcesses.filter((p) => p.user === g.id).length;
        })
      }
  ...
  async getByUuid(req, res, next) {
    try {
      ...
      if (search.length === 0) {
        return next(new HttpError(null, {}, NOT_FOUND, `Cannot found uge ${uuid}`));
      }
