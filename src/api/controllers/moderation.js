...
class Controller {
  async listModeration(req, res, next) {
      try {
        ...
        const { count, emotes } = await userEmotesService.getModerationEmotes(params);
        const emotess = await storeService.getEmotes(emotes);
        
        const emotesWithVideo = await Promise.all(emotess.map(async (e) => {
          ...
          return {
            ...
            data: {
              ...e.data,
              defaultVideoUrl: video.files.find(f => f.name === 'prepared').url
            }
          }
        }));
  ...
  async rankModeration(req, res, next) {
    try {
      ...
      await kinetixService.updateAsset({ uuid: emoteUuid, data: { metadata: { moderation: { rank } } } });
      await Promise.all([
  ...
  async validateModeration(req, res, next) {
    try {
      ...
      await kinetixService.updateAsset({ uuid: emoteUuid, data: { metadata: { moderation: { validated: true } } } });
      await Promise.all([
        cacheService.delEmotesVWUser(vw.uuid, userId),
        cacheService.delEmoteMetadata(vw.uuid, emoteUuid),
      ]);
