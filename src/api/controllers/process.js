...
import processHelper from "#common/services/processHelper.js";
...
const generateErrorMessage = (p) => {
  if (p?.mlReturnCode === 5)
    return 'Too much characters detected in the video'
  if (p?.mlReturnCode === 6)
    return 'No character detected in the video'
  if (p?.mlReturnCode === 7)
    return 'Character too small in the video'
}
...
class Controller {
  ...
  async delete (req, res, next) {
    try {
      ...
      if (!ALLOW_STEPS.includes(process.step)) {
        return next(new HttpError(null, {}, BAD_REQUEST, 'You can not interrupt an ongoing process'));
      }
  ...
  async getAllByCognito(req, res, next) {
    try {
      ...
      const enrichProcesses = await Promise.all(processes.toJSON().map((p) => enrichProcess(p)));
  ...
  async devRequest(req, res, next) {
    try {
      const { query } = req;
      const { userId, key } = query;
      
      const vw = key ? await vwService.getByKey(key) : await vwService.getBy({ id: 1 });
      const uuid = uuidv4();
      ...
      await cacheService.set(`sdk-token-${uuid}`, { vwId: vw.id, userId, keyId: vw.keyId }, parseInt(UGE_TOKEN_EXPIRE))
      let expire = new Date();
      expire.setSeconds(expire.getSeconds() + parseInt(UGE_TOKEN_EXPIRE));
      expire = expire.toISOString()
      const qrCode = await QRCode.toDataURL(`${VWC_URL}/?token=${uuid}&expire=${expire}`);
      let img = Buffer.from(qrCode.match('data:(image/.*);base64,(.*)')[2], 'base64');
      res.writeHead(200, {
  ...
  async request(req, res, next) {
    try {
      ...
      if (!vw.configuration.allowUGC) {
        return next(new HttpError(null, {}, FORBIDDEN, 'Virtual world does not allow UGC', 'noUGC'));
      }
      const { userId } = query
      const uuid = uuidv4();
      const hasUser = await vwService.hasUser(vw, userId);
  ...
  async create(req, res, next) {
    try {
      ...
      if (videoUuid) {
        try {
          ...
          if (videoAsset.type !== 'video') {
            return next(new HttpError(null, {}, BAD_REQUEST, 'uuid is not an video uuid'));
          }
          extension = videoAsset.files.find((f) => f.name === 'original')?.extension || 'mp4'
          name = body.name || videoAsset.name
        }
        catch (error) {
          logger.error(error.message, error);
          return next(new HttpError(null, {}, NOT_FOUND, `Cant find video uuid ${videoUuid}`))
        }
      ...
      const originalPath = videoAsset ? `user-data/${videoAsset.uuid}-original.${extension}` : '';
      const preparedPath = videoAsset ? `user-data/${videoAsset.uuid}-prepared.mp4` : '';
      ...
      if (!text) {
        try {
          ...
          await awsService.transcodeVideo({
            ...
            preparedPath,
            video,
            ...(user && user.sub && { thumbnail: `user-data/${videoAsset.uuid}-thumbnail.jpg` }),
            options: {
              start,
              end,
            }
  ...
  async videoInfo (req, res, next) {
    try {
      ...
      const {formats, thumbnails, automatic_captions, ...info} = await awsService.lambdaYoutube({ url: req.body.url })
  ...
  async videoDownload (req, res, next) {
    try {
      ...
      // CREATE ASSET VIDEO
      await awsService.lambdaYoutube({ url: req.body.url, outputPath: `s3://${S3_BUCKET}/user-data/${videoAsset.uuid}-original.mp4` });
  ...
  async validate(req, res, next) {
    try {
      ...
      if (!vw.configuration.ugcValidation) {
        return next(new HttpError(null, {}, FORBIDDEN, 'This route is disabled when validation flow is disabled. Check settings.'));
      }
      const process = await dynamoService.getProcess(uuid);
      if (!process) {
        return next(new HttpError(null, {}, NOT_FOUND, 'Process not found'));
      ...
      if (!process.step.includes('done')) {
        logger.error(`Tried to validated unvalidable process ${process.uuid} : ${process.step}`)
        return next(new HttpError(null, {}, FORBIDDEN, 'Process is not ready to be validated'));
      }
  ...
  async retake(req, res, next) {
    try {
      ...
      while(lastChildProcess.hierarchy?.child?.uuid || lastChildProcess.child?.uuid) {
        lastChildProcess = lastChildProcess.hierarchy?.child || lastChildProcess.child
      }
      if (lastChildProcess !== process) {
        lastChildProcess = await dynamoService.getProcess(lastChildProcess.uuid);
        lastChildProcess = await processHelper.buildHierarchy(lastChildProcess, false);
      }
      
      if (lastChildProcess.hierarchy.parents >= vw.configuration.ugcValidation.maxRetry) {
        return next(new HttpError(null, {}, NOT_FOUND, 'Process retake limit reached'));
      }
      ...
      await cacheService.set(`sdk-token-${token}`, { vwId: vw.id, userId: user.virtualWorldId, parent: lastChildProcess.uuid, keyId: vw.keyId }, parseInt(UGE_TOKEN_EXPIRE));
      let expire = new Date();
      expire.setSeconds(expire.getSeconds() + parseInt(UGE_TOKEN_EXPIRE));
      expire = expire.toISOString()
      const url = vw.configuration.ugcFrontentUrl || VWC_URL;
  ...
  async update(req, res, next) {
    try {
      ...
      const updatedProcess = await dynamoService.updateProcess(params.uuid, { name })
