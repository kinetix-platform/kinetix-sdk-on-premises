...
const { NOT_FOUND, FORBIDDEN, INTERNAL_SERVER_ERROR, OK, ALREADY_REPORTED } = httpStatus;
...
class Controller {
  ...
  async addEmote(req, res, next) {
    try {
      ...
      if (cachedData) {
        ...
        if (hasEmote) {
          return next(new HttpError(null, {}, OK, 'Emote already attributed to user'));
        }
      } 
      ...
      if (!provider) {
        return next(new HttpError(null, {}, FORBIDDEN, 'Emote not available on SDK', 'emoteNotAvailable'));
      }
      
      const isEmoteAllowed = await userService.isEmoteAllowed(user, emoteUuid);
      if (!isEmoteAllowed) {
        return next(new HttpError(null, {}, FORBIDDEN, 'Emote not allowed', 'emoteNotAllowed'));
      }
      ...
      if (vw.keyId)
        await monitorService.monitor(vw, 'mae')
      if (!(await monitorService.userHasActivityThisMonth(user)) && vw.keyId) {
        await monitorService.monitorUserActivity(user);
        await monitorService.monitor(vw, 'maeu')
      }

      await cacheService.delEmotesVWUser(vw.uuid, userId);
      
      res.send({ status: 'success', message: 'Emote successfully attributed to user.'});
    } catch (e) {
  ...
  async removeEmote(req, res, next) {
    try {
      ...
      const hasEmote = await userService.hasEmote(user, emoteUuid);
      if (!hasEmote) {
        return next(new HttpError(null, {}, NOT_FOUND, 'Emote not attributed to user'));
      }

      await userService.removeEmote(user, emoteUuid, remover?.isAdmin || false, reason);
      await Promise.all([
        cacheService.delEmotesVWUser(vw.uuid, userId),
        cacheService.delEmoteMetadata(vw.uuid, emoteUuid),
      ]);
      
      res.send({ status: 'success', message: 'Emote successfully removed from user.'});
    } catch (e) {
      logger.error('Error during addEmote', e);
      return next(new HttpError(null, e, INTERNAL_SERVER_ERROR, 'An error occured'));
    }
  ...
  async getEmotes(req, res, next) {
    try {
      ...
      if (cachedData && !since && !until) {
        ...
        res.status(ALREADY_REPORTED).send(cachedData.slice(offset, offset + limit))
      } else {
        const [user] = await vw.getUsers({where: { virtualWorldId: userId }});
        if (!user) {
          return next(new HttpError(null, {}, NOT_FOUND, 'User not found.'));
  ...
  async updateEmote(req, res, next) {
    try {
      ...
      if (!userHasEmote) {
        return next(new HttpError(null, {}, FORBIDDEN, 'User does not own the specified emote.'));
      }
  ...
  async getProcesses(req, res, next) {
    try {
      ...
      const processes = await processesServce.getProcessesByUser(user, ongoingOnly);
      const statusMap = {
        'pending': ['initialized', 'transcode_done', 'waiting_scheduler', 'requesting_ml', 'waiting_runner'],
        'done': ['render_done', 'done', 'validated', 'rejected'],
        'failed': ['transcode_failed', 'ml_request_failed', 'render_failed', 'association_failed'],
      }
      const getStatus = (process) => {
        ...
        return Object.entries(statusMap).reduce((acc, [key, values]) => {
          if (values.includes(process.step))
            acc = key;
          return acc;
        }, 'processing');
      }
  ...
  async getProcessesV2(req, res, next) {
    try {
      const { params, vw, query } = req;
      const { userId } = params;
      const { processing, pending, done, failed, validated, rejected, since } = query;
      const [user] = await vw.getUsers({where: { virtualWorldId: userId }});
      if (!user) {
        return next(new HttpError(null, {}, NOT_FOUND, 'User not found.'));
      }
      const statusMap = {
        ...
        'failed': ['transcode_failed', 'ml_request_failed', 'render_failed', 'association_failed', 'ml_failed'],
        'validated': ['validated'],
        'rejected': ['rejected'],
      }
      let options = {
      ...
      options.excludeSteps = options.excludeSteps.reduce((acc, stepToExclude) => {
        if (options.includeSteps.includes(stepToExclude)) return acc;
        acc.push(stepToExclude);
        return acc;
      }, [])
      
      const processes = await processesServce.getProcessesByUserV2(user, options);
      const getStatus = (process) => {
        if (process.validated) {
          return 'validated';
        }
