...
const { FORBIDDEN, INTERNAL_SERVER_ERROR, OK, NO_CONTENT, BAD_REQUEST, NOT_FOUND, PRECONDITION_REQUIRED, CONFLICT } = httpStatus;
...
class Controller {
  async createKey(req, res, next) {
    try {
      const { body, vw } = req;
      const {
        canRead, canWrite, expire
      } = body;
      
      if (expire && new Date(expire) <= new Date()) {
        return next(new HttpError(null, {}, BAD_REQUEST, 'Expire must be in the future'));
      }
      ...
      if (notDeletedKeys.length >= (MAX_VW_KEYS_COUNT || 1)) {
        return next(new HttpError(null, {}, FORBIDDEN, 'Maximum allowed keys.'));
      }
  ...
  async listUsage(req, res, next) {
    try {
      ...
      let usages = await vw.getUsages({ order: [['periodStart', 'DESC']], where: { planId: plan.id }});
      ...
      const caches = await cacheService.scan(`*usages:*:${vw.uuid}:${startOfMonth.unix()}:${plan.id}:*`)
      ...
      if (cachedUsages.length) {
        ...
        usages = usages.map(usage => {
          cachedUsages.forEach(cachedUsage => {
            if (cachedUsage.planId == usage.planId &&
        ...
        usages = [ ...usages, ...cachedUsages.filter(cachedUsage => !cachedUsage.merged)];
      }
      if (!usages.length) {
        res.send([]);
      } else {
  ...
  async list(req, res, next) {
    try {
      const { user } = req;
      const virtualWorlds = await vwService.getAll({ cognitoUuid: user.username, isDeleted: false });
  ...
  async create(req, res, next) {
    try {
      ...
      const existingVirtualWorlds = await vwService.getAll({ cognitoUuid: user.username, isDeleted: false });
      if (existingVirtualWorlds.length >= (MAX_VW_COUNT || 1)) {
        return next(new HttpError(null, {}, FORBIDDEN, 'Maximum allowed virtual worlds.'));
      }
  ...
  async update(req, res, next) {
    try {
      ...
      console.log(storedVW?.configuration?.defaultAvatarUuid !== defaultAvatarUuid)
  ...
  async createUser(req, res, next) {
    try {
      ...
      if (!user) {
        return next(new HttpError(null, {}, CONFLICT, 'User already exists', 'alreadyExists'));
      }
  ...
  async addEmotes(req, res, next) {
    try {
      ...
      const resp = await Promise.allSettled(uuids
        .map(async (uuid) => {
          try {
            ...
            const cause =
              e.response?.statusText === 'Not Found' ? 'Emote does not exist' :
              e.original.detail.includes('already exists') ? 'Emote already associated' :
              'Unknown error'
  ...
  async getEmotes(req, res, next) {
    try {
      ...
      if (USE_WHITELIST) {
        ...
        const emotesData = await storeService.getEmotes(emotes, vw, { mature, categories });
  ...
  async updatePlan(req, res, next) {
    try {
      ...
      if (targetPlan.name === "Unlimited") {
        ...
        if (!billingInfo) {
          return next(new HttpError(null, {}, FORBIDDEN, 'Billing info must be specified to upgrade plan'));
        }
      }
  ...
  async getAllAlias(req, res, next) {
    try {
      const { vw } = req;
      const { orderBy = 'createdAt', orderDirection = 'DESC', name } = req.query;
      ...
      const aliasesData = await Promise.all(aliases.map(async (alias) => {
        const aliasData = alias.dataValues;
        return aliasData;
      }));
    ...
    try {
      ...
      try {
        if (emotes.length) {
          ...
          await Promise.all(emotes.map((emote) => new Promise(async(resolve) => {
          ...
          if (unAuthorizedEmotes.length) {
            return next(new HttpError(null, {}, FORBIDDEN, `${unAuthorizedEmotes} are not authorized`))
          }
        }
  ...
  async updateAlias(req, res, next) {
    try {
      ...
      const { aliasUuid } = params;
      const { name, startDate, interval, description, emotes, categories, mature } = body;
      const [alias] = await vw.getAlias({ where: { uuid: aliasUuid }});
      if(!alias) {
        return next(new HttpError(null, {}, NOT_FOUND, 'Alias not found'));
      }
  ...
  async getAliasEmote(req, res, next) {
    try {
      ...
      if (alias.emotes.length === 0) {
        return next(new HttpError(null, {}, NOT_FOUND, `no emote is available for alias ${name}`));
        ...
        if (!(alias.interval && alias.startDate)) {
          return next(new HttpError(null, {}, PRECONDITION_REQUIRED, `You need to setup the interval & startDate for ${name}`));
        }

        const currentDate = new Date();
        const timePassed = currentDate - new Date(alias.startDate);
        const index = Math.floor(timePassed / alias.interval) % alias.emotes.length;
