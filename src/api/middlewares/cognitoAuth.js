...
const { COGNITO_USER_POOL_ID, COGNITO_CLIENT_ID, COGNITO_ALLOWED_GROUPS } = process.env;
...
export default async (req, res, next) => {
  ...
  try {
    ...
    const accessToken = cookies.kinetixCognito || headers.authorization?.split(' ')?.[1];

    payload =  await verifier.verify(accessToken);
    if (COGNITO_ALLOWED_GROUPS !== '*') {
      const groups = COGNITO_ALLOWED_GROUPS?.split(' ');
      const forbidden = !groups?.some(group => payload['cognito:groups']?.includes(group));
      if (forbidden) {
  ...
  try {
    ...
    const impersonation = await cacheService.scan(`impersonate:${payload.username}:*`);
    ...
    logger.error(`Error impersonating for user ${req.user.username} : ${e.message}`);
    return next(new HttpError(null, {}, UNAUTHORIZED, e.message));
  }
  try {
    await cognitoAuthService.createOrUpdate(req.user.username);
  } catch (e) {
    logger.error(`Error logging cognito authentication for user ${req.user.username} : ${e}`)
  }
  return next();
};
