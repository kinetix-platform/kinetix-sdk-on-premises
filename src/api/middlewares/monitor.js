...
const hasReachedLimitForMetric = (vw, usage, metric) =>
  usage && usage[metric] >= (vw.plan[limitMap[metric]] || Infinity);

export default (metric, monitor = true) =>
  async (req, res, next) => {
    const { vw } = req;
    try {
      ...
      if (vw.plan.name === "Limited") {
        ...
        const storedUsages = await vw.getUsages({
          where: { planId: vw.plan.id },
        });
        totalUsage = mergeUsages([
          ...cachedUsages,
          ...(storedUsages
      ...
      if (
        hasReachedLimitForMetric(vw, totalUsage, metric) ||
        hasReachedLimitForMetric(vw, vw.usage, metric)
      ) {
        return next(
          new HttpError(
            null,
