...
class Controller {
  ...
  async aggregateTotal(req, res, next) {
    try {
      ...
      const ugeEmotes = await sequelize.query(ugeEmotesCount([]), {
        type: QueryTypes.SELECT,
      });
      const ugeEmotesDeleted = await sequelize.query(ugeEmotesCount([], true), {
        type: QueryTypes.SELECT,
      });
      const result = {
  ...
  async aggregateRatio(req, res, next) {
    try {
      const gamesWithKey = await sequelize.query(gameWithKeyPercentage([]), {
        type: QueryTypes.SELECT,
      });
      const accWithUGR = await sequelize.query(accountsWithUGE([]), {
        type: QueryTypes.SELECT,
      });
