...
const allowedHeaderParts = ["ecs_", "x-amz", "x-forwarded-", "cloudfront-"];

const isHeaderAllowed = (header) => {
  return (
    allowedHeaders.includes(header) ||
    allowedHeaderParts.some((h) => header.includes(h))
  );
};
...
export default async (req, res, next) => {
  ...
  res.once("finish", () => {
    ...
    const { headers, body, query, originalUrl, ip, method, user, vw } = req;
    const { error } = res;
