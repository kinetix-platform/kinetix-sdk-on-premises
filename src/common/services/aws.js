...
const { NOT_ACCEPTABLE, REQUEST_ENTITY_TOO_LARGE, UNPROCESSABLE_ENTITY, NOT_FOUND } = httpStatus;
...
class AWSService {
  ...
  async transcodeVideo({
    ...
    if (start && end) {
      ...
      const durationInMilliseconds = differenceInMilliseconds(endTime, startTime);
  ...
  async getCognitoUserInfo(uuid) {
    ...
    formatted['cognito:groups'] = groupsData.Groups.map(group => group.GroupName)
  ...
  async lambdaYoutube({ url, outputPath }) {    
    ...
    if (response.FunctionError === 'Unhandled') {
      ...
      if (message.includes('The video is too long')) {
        throw new HttpError(null, {}, REQUEST_ENTITY_TOO_LARGE, 'Video is too long', 'videoTooLong')
      } else if (message.includes('The video is mature')) {
        throw new HttpError(null, {}, NOT_ACCEPTABLE, 'Video mature are not authorized', 'videoMature')
      } else if (message.includes('Unsupported URL')) {
        throw new HttpError(null, {}, UNPROCESSABLE_ENTITY, 'Unsupported URL', 'unsupportedURL')
        ...
        throw new HttpError(null, {}, NOT_FOUND, 'Not found', 'videoNotFound')
          ...
          throw new HttpError(null, {}, UNPROCESSABLE_ENTITY, 'Bad URL', 'badURL')
      } else if (message.includes('Not an video url')) {
        throw new HttpError(null, {}, UNPROCESSABLE_ENTITY, 'Not an video url', 'unsupportedURL')
      } else if (message.includes('youtube:truncated_id')) {
        throw new HttpError(null, {}, UNPROCESSABLE_ENTITY, 'Bad URL', 'badURL')
      } else if (message.includes('time out')) {
        throw new HttpError(null, {}, UNPROCESSABLE_ENTITY, 'Service time out', 'timeOut')
      } else if (message.includes('Unable to download webpage')) {
        throw new HttpError(null, {}, UNPROCESSABLE_ENTITY, 'Unsupported URL', 'unsupportedURL')
      } else if (message.includes('please report this issue')) {
        throw new HttpError(null, {}, UNPROCESSABLE_ENTITY, 'An error occured during video extraction', 'unsupportedURL')
      } else {
        throw new Error(message);
      }
    }
  ...
  async lambdaEmoteMeshRemover({ sourcePath, destinationPath, unreal = false, changeScale = true }) {
    const payload = {
      sourcePath,
      destinationPath,
      unreal,
