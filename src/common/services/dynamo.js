...
class DatabaseService {
  ...
  async getAll({ start, end, vw, pageSize = 50 }) {
    ...
    do {
      ...
      if (start && end) {
        query
          .where("createdAt")
          .between(moment(start).valueOf(), moment(end).valueOf());
      }
  ...
  async getProcessesByUserV2(user, options = {}) {
    let { includeSteps = [], excludeSteps = [], limit, limitDate } = options;
    ...
    if (conditionElements) {
      ...
      else query.and().parenthesis(condition);
    }
    if (limitDate) {
  ...
  async updateOldItems() {
    try {
      ...
      let updated = 0;
      const emptyFields = oldItems.filter(
        (item) => item.validated === undefined || item.rejected === undefined,
      );
