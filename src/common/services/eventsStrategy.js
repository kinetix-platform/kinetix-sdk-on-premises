...
export default {
  ...
  "enforce-usage": async ()   => {
    ...
    try {
      ...
      await Promise.all(Object.entries(usageKeys).map(async ([vwUuid, vwKeys]) => {
        ...
        await Promise.all(Object.entries(keysByGroup).map(async ([groupString, groupKeys]) => {
          ...
          const storedUsage = await usageService.getAll({ periodStart: moment.unix(timestamp).utcOffset(0), planId, virtualWorldId: vw.id, keyId }, ['*'], { raw: true, order: [['created_at', 'DESC']] })
          const instanceValues = await Promise.all(groupKeys.map(async (rawKey) => {
          ...
          await Promise.all(groupKeys.map(async (rawKey) => {
            const key = rawKey.split(':').slice(1).join(':');
