...
class Kinetix {
  ...
  async createAsset({ cognitoUuid, files = [], ...asset }) {
    ...
    const r = await axios.post(route, body, {
      headers: {
  ...
  async updateAsset({ uuid, data }) {
    ...
    const r = await axios.put(route, body, {
      headers: {
        "Content-Type": "application/json",
        Authorization: `hmac signature=${signature}`,
      },
  ...
  async deleteAsset(uuid) {
    ...
    const r = await axios.delete(route, {
      headers: {
        "Content-Type": "application/json",
        Authorization: `hmac signature=${signature}`,
      },
  ...
  async getAvatars(cognitoUuid) {
    ...
    const avatars = r?.data
      ? r.data.data.filter(({ metadata }) =>
          ["Validated", "Processed"].includes(metadata?.status),
        )
      : [];
