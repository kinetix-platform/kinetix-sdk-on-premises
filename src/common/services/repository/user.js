...
export class UsersService extends CrudService {
  ...
  async getAllFromVW(vw) {
    const response = await this.model.findAll({
      attributes: {
        include: [[Sequelize.fn("COUNT", Sequelize.col("emotes.id")), "emotesCount"], '*']
      },
      include: [{
        model: VWModel,
        as: 'virtualWorlds',
      ...
      order: [['emotesCount', 'DESC'], ['created_at', 'DESC']],
    });
   ...
   */
  async getEmotes(user, { since, until, limit, offset, order, uuids, raw = true }) {
    const params = {
      where: {
        userId: user.id,
        isDeleted: false,
  ...
  async rankEmote(user, emoteUuid, rank) {
    const emote = await this.hasEmote(user, emoteUuid);
    if (emote) {
      emote.moderation = emote.moderation ? { ...emote.moderation, rank } : { rank };
      await emote.save();
    }
    return emote;
  }
  ...
  async validateEmote(user, emoteUuid, asAdmin = false) {
    ...
    if (emote) {
      ...
      emote.changed('validatedAt', true);
      emote.moderation = emote.moderation ? { ...emote.moderation, validated: true } : { validated: true };
      await emote.save();
    }
  }
}
