...
export const format = (ms) =>
  moment.duration(ms, "milliseconds").format("h [hrs], m [min], s [sec]");

export const processesStats = (totalProcesses) => {
  const success = totalProcesses.filter((p) =>
    [0, 5, 6, 7].includes(p.mlReturnCode),
  );

  const failed = totalProcesses.filter(
  ...
  const totalSum = success.reduce(
    (acc, cur) => cur.mlEndedAt - cur.createdAt + acc,
    0,
  );
  ...
  const spawnSum = success.reduce(
    (acc, cur) => cur.mlStartedAt - cur.createdAt + acc,
    0,
  );
  ...
  const processingDiffArray = success.map(
    (cur) => cur.mlEndedAt - cur.mlStartedAt,
  );
  const processingSum = success.reduce(
    (acc, cur) => cur.mlEndedAt - cur.mlStartedAt + acc,
    0,
  );
  ...
  return {
    ...
    successRate: totalProcesses.length
      ? success.length / totalProcesses.length
      : null,
    ventillProcessStats,
    totalTime: {
      average: {
