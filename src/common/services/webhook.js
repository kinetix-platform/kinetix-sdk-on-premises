  ...
  async call(vw, process, status, extra) {
    ...
    try {
      if (vw.configuration.webhookUri) {
        await axios.post(vw.configuration.webhookUri, { uuid, user: user.virtualWorldId, status, emote, name, ...extra })
        logger.info(`process ${uuid} notify ${vw.configuration.webhookUri}`)
      }
    } catch (error) {
      logger.error(`process ${uuid} ${status} cannot notify webhook ${vw.configuration.webhookUri}`, { message: error.message });
    }
  }
}
